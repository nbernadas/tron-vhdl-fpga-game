...
architecture func of opp_reg is
...
begin
	process (clk50, nrst) begin
		if nrst = '0' then st <= wait_init;
			...
			case st is
				when wait_init => if ast = '1' then 
									if switch = '0' then st <= p1_init;
									else st <= p2_init;
									end if;
								  end if;
				when p1_init => if read_move = '1' then st <= wait_move; end if;
				when p2_init => if read_move = '1' then st <= wait_move; end if;
	...
	movt_new <= '1' when st = ack_game or st = p1_init or st = p2_init else '0';
	key_read <= '1' when st = ack_game or st = p1_init or st = p2_init else '0';
